 ...
 */
JNIEXPORT void JNICALL Java_com_clumba_cxxapp01_Ner_init
(JNIEnv * env, jobject, jstring jrootPath){

    std::string rootPath = env->GetStringUTFChars(jrootPath, nullptr);
    std::string modelPath = rootPath + "/export-trace.pt";
    std::string vocabPath = rootPath +"/words.txt";
    std::string tagPath = rootPath +"/tags.txt";
 ...
 */
JNIEXPORT jstring JNICALL Java_com_clumba_cxxapp01_Ner_decode
...
std::vector<std::string> split(std::string str, std::string separator){
	...
	for (int i=0;i<length;i++){
		...
		if(pos<length){
			std::string s = str.substr(i,pos-i); //包头不包尾,(start,length)
